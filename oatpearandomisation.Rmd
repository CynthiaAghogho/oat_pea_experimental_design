...
gSeedPer60sqftplot_pea <- 59
gSeedPer60sqftplot_oat <- 24

inventory %<>% 
  mutate(
    N_60sqftplotPossible = case_when(
      crop == "oat" ~ weight/gSeedPer60sqftplot_oat,  # Calculate for oats
      crop == "pea" ~  weight /gSeedPer60sqftplot_pea  # Calculate for peas
    ),
    # Assign values based on how many plots can be accommodated by seeds provided
    PlotCategory = case_when(
      N_60sqftplotPossible >= 6 ~ "5", # Can accommodate at least 10 plots
      N_60sqftplotPossible >= 2  ~ "2", # Can accommodate at least 2 plots but less than 10
...
combined_counts  %<>% 
  mutate(
    N_60sqftplotneeded = case_when(
      crop == "Oat" ~ Count*gSeedPer60sqftplot_oat,  # Calculate for oats
      crop == "Pea" ~  Count*gSeedPer60sqftplot_pea  # Calculate for peas
    ))

inventory <- inventory [c("Name","weight")]  
final_data_count <- left_join(combined_counts, inventory, by = c("accessions" = "Name"))
final_data_count1 <- final_data_count  %<>% 
  mutate(Check = ifelse(N_60sqftplotneeded > weight, FALSE, TRUE))
